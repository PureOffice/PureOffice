...
def gen_allfonts(fonts_infos, font_files_all, font_ranges):
    """生成 sdkjs/common/AllFonts.js —— 引擎 CheckAllFonts 契约（POC 同方案）。
    注意：字体名必须 JSON 引号（此前 str() 生成裸 [Liberation Sans,...] 实机 SyntaxError）
    三表注入（__fonts_files/__fonts_infos/__fonts_ranges）与用户字体 URL 注册
    （lsofonts 参数段）原在 ascshim 20_bridge 头部——AllFonts.js 由官方链保证
    早于 sdk-all.js（checkAllFonts 前）加载，注入时机等价（2026-09-21 阶段 2-j1）"""
    import json
    sel_b64 = make_font_selection_bin([(n[0], n[1]) for n in fonts_infos])
    user_fonts_reg = r'''
// [OHOS: userfonts] 用户自导入字体注册（原 ascshim 20_bridge 0.2 段）：URL 参数
...
(function () {
  try {
    ...
    var _n = 0;
    var _skip = 0;
    // 名字清单供字体下拉归一（apiBase.js [OHOS: fonts]）与字典补录（ohos/bridge.js）
    // 使用——用户字体不参与族归一（无随包缩略图，并入族组会被丢）。
    window.__lso_user_font_names = window.__lso_user_font_names || [];
    // 空白缩略图槽位索引 = 追加前的内置行数（精灵图尾部多留一格，见 make_fonts_sprites）
...
'''
    return ('// AllFonts.js (generated by build_editors_ohos.py)\n'
            ...
            # 三表（Emumerator checkAllFonts 唯一入口；顺序 R,I,B,BI=indexI/B 数组下标；
            # ranges=字符范围回退注册表，无它 CJK fallback 永失败——2026-09-05 根因）
            'window["__fonts_files"] = %s;\n' % json.dumps(font_files_all) +
            'window["__fonts_infos"] = %s;\n' % json.dumps(fonts_infos) +
            'window["__fonts_ranges"] = %s;\n' % json.dumps(font_ranges) +
            user_fonts_reg)
...
def gen_smoke_head():
    """smoke/src/*.js 三段拼装 → rawfile/onlyoffice/smoke/inject-head.js（验收态
    head 注入脚本——原 ascshim 尾段域，2026-09-21 阶段 2-n 迁入 smoke）。

    段序（= 原 ascshim PARTS 序）：
      fontpick.js  字体映射探针（onLogPickFont 官方回调——须早于引擎首次解析
                   字体名，head 注入即满足）
      boot.js      外层 IIFE 头（**故意不闭合**）+ __lsoB64 工具 + 插件 fetch 取证
      m7open.js    m7 验收三块（m7auto/m7img/m7open，URL 参数门控）+ 外层 IIFE 闭合

    产品 URL 不带 m7 参数 → 各段门控早退 = 零行为（页面仅多一次脚本加载）。
    node --check 硬校验（失败非零退出）。**必须在 install_smoke 之后调用**——
    其 rmtree(SMOKE_DST) 会清掉本产物。
    """
    parts = ['fontpick.js', 'boot.js', 'm7open.js']
    src_dir = os.path.join(SMOKE_SRC, 'src')
    js = ('/* auto-generated by build_editors_ohos.gen_smoke_head — do not edit\n'
          '   smoke 验收态 head 注入（源 scripts/onlyoffice/smoke/src/）*/\n')
    for p in parts:
        js += open(os.path.join(src_dir, p), encoding='utf-8').read()
    dst = os.path.join(SMOKE_DST, 'inject-head.js')
    with open(dst, 'w', encoding='utf-8') as f:
        f.write(js)
    import subprocess
    rc = subprocess.run(['node', '--check', dst], capture_output=True, text=True)
    if rc.returncode != 0:
        raise SystemExit('node --check FAILED on %s:\n%s' % (dst, rc.stderr[:2000]))
    print('  smoke/inject-head.js 生成（%d bytes，%d 段）' % (os.path.getsize(dst), len(parts)))
...
def main():
    ...
    # 4. 各 app main/index.html 注入 smoke/inject-head.js（验收态 head 脚本，
    #    原 ascshim——2026-09-21 阶段 2-n 退役改名；产物由 7 节 gen_smoke_head 生成）
    injected = 0
    for app in APP_MAIN:
    ...
    # 2026-09-05 审查补）
    if injected != 4:
        raise SystemExit('smoke head 注入页数 %d != 4：web-apps deploy 产物不完整或已注入过旧版本'
                         % injected)
    ...
    with open(allfonts_dst, 'w', encoding='utf-8') as f:
        f.write(gen_allfonts(FONT_INFOS, FONT_FILES_ALL, FONT_RANGES))
    ...
    if os.path.isfile(LOGIN):
        ...
        inject_script_src(dst, 'smoke/inject-head.js')
        # 欢迎页 bridge（AscDesktopEditor 方法表供给——loginpage 的 sdk 面板链
        # LocalFileRecents/Recovers 与 2.5/2.6 recents 桥都依赖；INSTALL 内各
        # 编辑器页专属块自带页判，欢迎页安全）。注：此注入曾误放 patch_about_brand
        # 死函数内从未执行（2-j2 事故，ascshim 尚存 INSTALL 时被掩盖未暴露）——
        # ascshim 退役后欢迎页失去桥供给，2026-09-21 阶段 2-n 移回本活代码处
        inject_script_src(dst, 'ohos/bridge.js')
    ...
    install_smoke()
    # 7.05 验收态 head 注入脚本（原 ascshim 尾段域，2026-09-21 阶段 2-n 迁入 smoke；
    #      必须在 install_smoke 之后——其 rmtree SMOKE_DST 会清掉本产物）
    gen_smoke_head()

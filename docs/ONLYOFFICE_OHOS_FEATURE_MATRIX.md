...
| 打开方式（文件管理器交付） | ✅ | `module.json5` 声明 `viewData` skill（`scheme=file` + **逐条枚举** formats 表 9 种格式的 UTD + `linkFeature=FileOpen`）→ 文件管理器「打开方式」候选出现 Pure Office → `want.uri` → `handleLocalUri` 与 picker 共用处理链；不支持格式弹「暂不支持该格式：&lt;名&gt;」（2026-09-12 真机 1.6：docx 进编辑器 / odt 被拦）。**刻意不用 `utd=general.entity` 通配**：UTD 根类型等于声称「能打开一切文件」，而壳层只支持 9 种——声明宽于能力会让打不开的类型也列出本应用，与「按实际能力配置 skills」的上架要求不符；UTD 值取真机日志 `normalized:<值>` 实测的规范化结果 |
...
- M1 官方构建链 ✅ / M2 桥 ✅ / M3 欢迎页+docx 打开 ✅（2026-09-04 上午，真机）

...
**前置验证（实施第一步，不通过即停）**：只加 + 按钮 + 三项菜单 + 打点，真机点一遍确认 `action` 回调触发。这是本设计唯一的未知数（§7）——**已通过**（见 §7 第一条）。
...
- **高（已排除）**：`bindMenu` 的 `action` 回调真机是否触发——本项目首次用系统菜单。CustomDialog 的前科是"自定义内容里的按钮 `onClick` 不触发"（事件经组件树分发），`bindMenu` 走系统菜单回调，机制不同。真机实测（PC/1.5）：点 + 弹出菜单后选类型，`action` 如实到达 ArkTS（`TAB_NEW_MENU ext=xlsx|pptx` → 新 tab 开出），三项全通。
  - **降级预案（未启用）**：自绘居中弹层（蒙层 + 居中卡片 + 三个选项，与既有 4 个弹层同构）。仅在换设备/SDK 后若发现 `action` 不通时启用，届时改 §3.2 的菜单部分，其余（位置、显示条件、回调、文案）不变。
